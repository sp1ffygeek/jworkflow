...
public class WorkflowExecutorTest {
  ...
  public void setup() {
    ...
    subject =
        new DefaultWorkflowExecutor(
            registry, resultProcessor, Clock.systemUTC(), Logger.global, injector);
  }
  ...
  public void should_trigger_step_hooks() throws Exception {
    ...
    verify(step1)
        .initForExecution(
            any(WorkflowExecutorResult.class),
            any(WorkflowDefinition.class),
            any(WorkflowInstance.class),
            any(ExecutionPointer.class));
    verify(step1)
        .beforeExecute(
            any(WorkflowExecutorResult.class),
            any(StepExecutionContext.class),
            any(ExecutionPointer.class),
            any(StepBody.class));
    verify(step1)
        .afterExecute(
  ...
  public void should_map_inputs() throws Exception {
    // arrange
    StepFieldConsumer<StepWithProperties, DataClass> p1 =
        (step, data) -> step.property1 = data.value1;
  ...
  public void should_map_outputs() throws Exception {
    // arrange
    StepFieldConsumer<StepWithProperties, DataClass> p1 =
        (step, data) -> data.value2 = step.property2;
  ...
  public void should_handle_step_exception() throws Exception {
    ...
    // assert
    verify(step1Body).run(any(StepExecutionContext.class));
    verify(resultProcessor)
        .handleStepException(
            any(WorkflowInstance.class),
            any(WorkflowDefinition.class),
            any(ExecutionPointer.class),
            any(WorkflowStep.class));
    verify(resultProcessor, never())
  ...
  private WorkflowStep buildFakeStep(StepBody stepBody)
      throws InstantiationException, IllegalAccessException {
    return buildFakeStep(stepBody, new ArrayList<>(), new ArrayList<>());
  }

  private WorkflowStep buildFakeStep(
      StepBody stepBody, List<StepFieldConsumer> inputs, List<StepFieldConsumer> outputs)
      throws InstantiationException, IllegalAccessException {
    ...
    when(result.initForExecution(
            any(WorkflowExecutorResult.class),
            any(WorkflowDefinition.class),
            any(WorkflowInstance.class),
            any(ExecutionPointer.class)))
        .thenReturn(ExecutionPipelineResult.NEXT);
    when(result.beforeExecute(
            any(WorkflowExecutorResult.class),
            any(StepExecutionContext.class),
            any(ExecutionPointer.class),
            any(StepBody.class)))
