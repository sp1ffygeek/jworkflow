...
public class ExecutionResultProcessorTest {
  ...
  public void setup() {
    ...
    subject =
        new DefaultExecutionResultProcessor(
            pointerFactory, errorHandlers, Clock.systemUTC(), Logger.global);
  }
  ...
  public void should_subscribe_to_event() {
    ...
    assertTrue(
        workflowResult.subscriptions.stream()
            .anyMatch(
                x -> x.stepId == pointer.stepId && x.eventName == "Event" && x.eventKey == "Key"));
  }
  ...
  public void should_branch_children() {
    ...
    when(pointerFactory.buildChildPointer(definition, pointer, child, branch))
