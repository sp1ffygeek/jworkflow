...
public class DefaultStepBuilder<TData, TStep extends StepBody>
    implements StepBuilder<TData, TStep>, ControlStepBuilder<TData, TStep> {

  private final WorkflowBuilder workflowBuilder;
  private final WorkflowStep step;
  private final Class<TData> dataClass;

  public DefaultStepBuilder(
      Class<TData> dataClass,
      Class<TStep> stepClass,
  ...
  @Override
  public <TNewStep extends StepBody> StepBuilder<TData, TNewStep> then(
    ...
    StepBuilder<TData, TNewStep> stepBuilder =
        new DefaultStepBuilder<>(dataClass, stepClass, workflowBuilder, newStep);

    if (stepSetup != null) stepSetup.accept(stepBuilder);
  ...
  public <TNewStep extends StepBody> StepBuilder<TData, TNewStep> then(
    ...
    StepBuilder<TData, TNewStep> stepBuilder =
        new DefaultStepBuilder<>(dataClass, stepClass, workflowBuilder, newStep.getStep());
  ...
  public StepBuilder<TData, WorkflowStepInline.InlineBody> then(StepExecutionConsumer body) {
    ...
    workflowBuilder.addStep(newStep);
    StepBuilder<TData, WorkflowStepInline.InlineBody> stepBuilder =
        new DefaultStepBuilder<>(
            dataClass, WorkflowStepInline.InlineBody.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
  ...
  public StepBuilder<TData, ConsumerStep> thenAction(Consumer<StepExecutionContext> body) {
    ...
    workflowBuilder.addStep(newStep);
    StepBuilder<TData, ConsumerStep> stepBuilder =
        new DefaultStepBuilder<>(dataClass, ConsumerStep.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
  ...
  @Override
  public StepBuilder<TData, WaitFor> waitFor(
    ...
    StepFieldConsumer<WaitFor, TData> keyConsumer =
        (step, data) -> step.eventKey = eventKey.apply(data);
    StepFieldConsumer<WaitFor, TData> dateConsumer =
        (step, data) -> step.effectiveDate = effectiveDateUtc.apply(data);
    newStep.addInput(nameConsumer);
    newStep.addInput(keyConsumer);
    newStep.addInput(dateConsumer);

    workflowBuilder.addStep(newStep);
    StepBuilder<TData, WaitFor> stepBuilder =
        new DefaultStepBuilder<>(dataClass, WaitFor.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public ControlStepBuilder<TData, Foreach> foreach(Function<TData, Object[]> collection) {
    ...
    StepFieldConsumer<Foreach, TData> collectionConsumer =
        (step, data) -> step.collection = collection.apply(data);
    newStep.addInput(collectionConsumer);
    workflowBuilder.addStep(newStep);
    ControlStepBuilder<TData, Foreach> stepBuilder =
        new DefaultStepBuilder<>(dataClass, Foreach.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public ControlStepBuilder<TData, While> While(Function<TData, Boolean> condition) {
    ...
    StepFieldConsumer<While, TData> conditionConsumer =
        (step, data) -> step.condition = condition.apply(data);
    newStep.addInput(conditionConsumer);
    workflowBuilder.addStep(newStep);
    ControlStepBuilder<TData, While> stepBuilder =
        new DefaultStepBuilder<>(dataClass, While.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public ControlStepBuilder<TData, If> If(Function<TData, Boolean> condition) {
    ...
    StepFieldConsumer<If, TData> conditionConsumer =
        (step, data) -> step.condition = condition.apply(data);
    newStep.addInput(conditionConsumer);
    workflowBuilder.addStep(newStep);
    ControlStepBuilder<TData, If> stepBuilder =
        new DefaultStepBuilder<>(dataClass, If.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public StepBuilder<TData, Delay> delay(Function<TData, Duration> duration) {
    ...
    StepFieldConsumer<Delay, TData> durationConsumer =
        (step, data) -> step.duration = duration.apply(data);
    newStep.addInput(durationConsumer);

    workflowBuilder.addStep(newStep);
    StepBuilder<TData, Delay> stepBuilder =
        new DefaultStepBuilder<>(dataClass, Delay.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public ControlStepBuilder<TData, Schedule> schedule(Function<TData, Duration> duration) {
    ...
    StepFieldConsumer<Schedule, TData> durationConsumer =
        (step, data) -> step.duration = duration.apply(data);
    newStep.addInput(durationConsumer);

    workflowBuilder.addStep(newStep);
    ControlStepBuilder<TData, Schedule> stepBuilder =
        new DefaultStepBuilder<>(dataClass, Schedule.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public ParallelStepBuilder<TData, Sequence> parallel() {
    ...
    StepBuilder<TData, Sequence> newBuilder =
        new DefaultStepBuilder(dataClass, Sequence.class, workflowBuilder, newStep);
    workflowBuilder.addStep(newStep);
    ParallelStepBuilder<TData, Sequence> stepBuilder =
        new DefaultParallelStepBuilder(workflowBuilder, newBuilder, newBuilder);
    step.addOutcome(newStep.getId(), null);
    return stepBuilder;
  }
  ...
  public StepBuilder<TData, Sequence> saga(WorkflowBuilderConsumer<TData> consumer) {
    ...
    workflowBuilder.addStep(newStep);
    StepBuilder<TData, Sequence> stepBuilder =
        new DefaultStepBuilder<>(dataClass, Sequence.class, workflowBuilder, newStep);
    step.addOutcome(newStep.getId(), null);
    consumer.accept(workflowBuilder);
  ...
  @Override
  public <TNewStep extends StepBody> StepBuilder<TData, TStep> compensateWith(
      Class<TNewStep> stepClass) {
