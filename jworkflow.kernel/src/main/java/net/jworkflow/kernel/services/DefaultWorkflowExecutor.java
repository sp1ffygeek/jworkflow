...
public class DefaultWorkflowExecutor implements WorkflowExecutor {
  ...
  public WorkflowExecutorResult execute(WorkflowInstance workflow) {
    ...
    if (workflow.getStatus() != WorkflowStatus.RUNNABLE) return wfResult;

    List<ExecutionPointer> exePointers = workflow.getExecutionPointers().findMany(x -> x.active);
    WorkflowDefinition def =
        registry.getDefinition(workflow.getWorkflowDefintionId(), workflow.getVersion());

    if (def == null) {
      logger.log(Level.SEVERE, "Workflow not registered");
      return wfResult;
    }

    for (ExecutionPointer pointer : exePointers) {

      Optional<WorkflowStep> step =
          def.getSteps().stream().filter(x -> x.getId() == pointer.stepId).findFirst();
      ...
      try {
        if (step.get().initForExecution(wfResult, def, workflow, pointer)
            == ExecutionPipelineResult.DEFER) continue;

        if (pointer.startTimeUtc == null) pointer.startTimeUtc = Date.from(Instant.now(clock));

        logger.log(
            Level.INFO,
            String.format(
                "Starting step %s on workflow %s", step.get().getName(), workflow.getId()));

        StepBody body = step.get().constructBody(injector);
        ...
        if (step.get().beforeExecute(wfResult, context, pointer, body)
            == ExecutionPipelineResult.DEFER) continue;
        ...
        resultProcessor.processExecutionResult(
            workflow, def, pointer, step.get(), result, wfResult);
    ...
    if (workflow.getNextExecution() == null) return wfResult;

    long now = new Date().getTime();
    wfResult.requeue =
        ((workflow.getNextExecution() < now) && workflow.getStatus() == WorkflowStatus.RUNNABLE);
  ...
  private void determineNextExecution(WorkflowInstance workflow) {
    ...
    for (ExecutionPointer pointer : workflow.getExecutionPointers()) {
      if ((pointer.active) && (pointer.children.isEmpty())) {
        ...
        workflow.setNextExecution(
            Math.min(
                pointerSleep,
                workflow.getNextExecution() != null ? workflow.getNextExecution() : pointerSleep));
      }
    }
    ...
    if (workflow.getNextExecution() == null) {
      for (ExecutionPointer pointer : workflow.getExecutionPointers()) {
        if ((pointer.active) && (!pointer.children.isEmpty())) {
          ...
          if (!workflow.isBranchComplete(pointer.id)) continue;
          ...
          workflow.setNextExecution(
              Math.min(
                  pointerSleep,
                  workflow.getNextExecution() != null
                      ? workflow.getNextExecution()
                      : pointerSleep));
        }
      }
    }
  ...
  private void processAfterExecutionIteration(
      WorkflowInstance workflow,
      WorkflowDefinition workflowDef,
      WorkflowExecutorResult workflowResult) {
    List<ExecutionPointer> pointers =
        workflow.getExecutionPointers().findMany(x -> x.endTimeUtc == null);
