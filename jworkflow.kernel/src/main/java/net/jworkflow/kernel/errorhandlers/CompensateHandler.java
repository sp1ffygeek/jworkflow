  ...
  public void handle(
    ...
    while (!scope.isEmpty()) {
      ...
      while (!txnStack.isEmpty()) {
        ...
        if ((!parentStep.getResumeChildrenAfterCompensation())
            || (parentStep.getRevertChildrenAfterCompensation())) {
      ...
      if ((ptrstep.getRetryBehavior() != ErrorBehavior.COMPENSATE)
          && (ptrstep.getRetryBehavior() != null)) {
      ...
      if (ptrstep.getCompensationStepId() != null) {
        ...
        ExecutionPointer compensationPointer =
            pointerFactory.buildCompensationPointer(
                def, ptr, pointer, ptrstep.getCompensationStepId());
              ...
              .forEach(
                  x ->
                      workflow
                          ...
                          .add(pointerFactory.buildNextPointer(def, ptr, x)));
        }
      }
      ...
      if (revert) {
        workflow
            ...
            .findMany(
                x ->
                    ptr.callStack.equals(x.callStack)
                        && !x.id.equals(ptr.id)
            ...
            .forEach(
                siblingPtr -> {
                  ...
                  if (siblingStep.getCompensationStepId() != null) {
                    ExecutionPointer compensationPointer =
                        pointerFactory.buildCompensationPointer(
                            def, siblingPtr, pointer, siblingStep.getCompensationStepId());
                    workflow.getExecutionPointers().add(compensationPointer);
